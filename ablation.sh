...
EVAL_COMPONENTS="gcomp:${BENCH}:0=c0,gcomp:${BENCH}:2=c2,gcomp:${BENCH}:3=c3"
...
run_guide () {
  ...
  "$PY" ablate_guide.py \
    ...
    --weights "0.333,0.333,0.334" --train_betas "$betas" \
...
if [[ "$WHICH" == "all" || "$WHICH" == "guide" ]]; then
  echo "==================== ablate_guide ===================="
  # flow (DB) guides
  run_guide "tempgain-ws-from-db-beta20-t1.0-e0.05-t1.0-e0.05" "20,20,20" flow   harmonic "${CKPT_ROOT}/osim-compose-db-20-tempgain/ablation"
  run_guide "tempgain-ws-from-db-beta20-t1.0-e0.05-t1.0-e0.05" "20,20,20" flow   poe      "${CKPT_ROOT}/osim-compose-db-20-tempgain/ablation-poe"
fi

if [[ "$WHICH" == "all" || "$WHICH" == "flip" ]]; then
  echo "==================== ablate_logit_flip ===================="
  # suffix          route   temp   out_dir
  run_flip "tempgain-ws-from-db-beta20-t1.0-e0.05-t1.0-e0.05" flow   1.0 "${CKPT_ROOT}/osim-compose-db-20-tempgain/flip"
  run_flip "tempgain-ws-from-db-beta20-t1.0-e0.05-t1.0-e0.05" flow   0.3 "${CKPT_ROOT}/osim-compose-db-20-tempgain/0.3-flip"
fi

if [[ "$WHICH" == "all" || "$WHICH" == "ceiling" ]]; then
  echo "==================== ablate_ceiling (#2 + #5) ===================="
  # suffix          route   betas         out_dir
  run_ceiling "tempgain-ws-from-db-beta20-t1.0-e0.05-t1.0-e0.05" flow   "20,20,20" "${CKPT_ROOT}/osim-compose-db-20-tempgain/ceiling"
fi

if [[ "$WHICH" == "all" || "$WHICH" == "singles" ]]; then
  echo "==================== ablate_singles_weights (#3 + #4) ===================="
  # suffix          route   kind     betas         out_dir
  run_singles "tempgain-ws-from-db-beta20-t1.0-e0.05-t1.0-e0.05" flow   harmonic      "20,20,20" "${CKPT_ROOT}/osim-compose-db-20-tempgain/singles-weights"
  run_singles "tempgain-ws-from-db-beta20-t1.0-e0.05-t1.0-e0.05" flow   poe      "20,20,20" "${CKPT_ROOT}/osim-compose-db-20-tempgain/singles-weights-poe"
fi

if [[ "$WHICH" == "all" || "$WHICH" == "diag_rollout" ]]; then
  echo "==================== diag_rollout (mechanisms 2,3,4) ===================="
  run_diag_rollout "tempgain-ws-from-db-beta20-t1.0-e0.05-t1.0-e0.05" flow   "${CKPT_ROOT}/osim-compose-db-20-tempgain/rollout-diag"
fi
...
if [[ "$WHICH" == "all" || "$WHICH" == "diag_logs" ]]; then
  ...
  if [[ -z "$WANDB_ENTITY" ]]; then
    ...
    for suf in tempgain-ws-from-db-beta20-t1.0-e0.05-t1.0-e0.05; do
      names=""
      for c in 0 2 3; do names+="gfn-geom-osim-comp${c}--${suf},"; done
      names="${names%,}"
      out="${CKPT_ROOT}/training-diag-tempgain/${suf}"
